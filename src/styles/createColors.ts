...
export type TextVariantUnion = 'primary' | 'secondary' | 'invert';
...
const completeSystemColor = (variantInput: Partial<IColorVariant> | string) => {
  ...
  if (typeof variantInput === 'string') {
    ...
    throw new Error(
      [
        '[DS]: The color format provided to `theme` is invalid!',
  ...
  const getContrastColorOf = (background: ColorInputType) => {
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      if (contrast < 3) {
        ...
        console.error(
          [
            `DS: The contrast ratio of ${contrast}:1 for ${contrastForeground} on ${background}`,
